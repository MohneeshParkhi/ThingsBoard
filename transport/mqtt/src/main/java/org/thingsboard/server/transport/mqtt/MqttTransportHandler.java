...
public class MqttTransportHandler extends ChannelInboundHandlerAdapter implements GenericFutureListener<Future<? super Void>> {
  ...
  // method will execute after running ProcessConnect method
    private void processPublish(ChannelHandlerContext ctx, MqttPublishMessage mqttMsg) {
        
        
        // return true if connected, method contain Vlaotile boolean varibale,a lready set as true in processConnect() method
        // conditional statement will not execute as true will return and Logical Not will make it as false.
        if (!checkConnected(ctx)) {
            return;
        }
        
        // getting Topic name set as part of message
        ...
        // MqttTopics.java class contain all topics names
        if (topicName.startsWith(BASE_GATEWAY_API_TOPIC)) {
            if (gatewaySessionCtx != null) {
                //setting channel to communicate
     ...
     // called from processPublish() method
    private void handleMqttPublishMsg(String topicName, int msgId, MqttPublishMessage mqttMsg) {
        // class hold various topic based condition to call particular methods....
